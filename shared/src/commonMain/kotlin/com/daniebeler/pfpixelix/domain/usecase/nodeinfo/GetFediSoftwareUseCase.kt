        ...
        countryRepository.getSoftwareFromFediDB(domain).collect { res ->
            ...
            if (res is Resource.Success) {
                ...
                if (res.data?.slug == "pixelfed") {
                    res.data.icon = -1 //todo R.drawable.pixelfed_logo
                } else if (res.data?.slug == "mastodon") {
                    res.data.icon = -1 //todo R.drawable.mastodon_logo
                } else if (res.data?.slug == "peertube") {
                    res.data.icon = -1 //todo R.drawable.peertube_logo
                } else if (res.data?.slug == "lemmy") {
                    res.data.icon = -1 //todo R.drawable.lemmy_logo
                } else if (res.data?.slug == "misskey") {
                    res.data.icon = -1 //todo R.drawable.misskey_logo
                } else {
                    res.data!!.icon = -1 //todo R.drawable.fediverse_logo
                }
                emit(res)
            }
        }
